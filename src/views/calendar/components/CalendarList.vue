  ...
  <div>
    ...
    <van-cell-group>
        ...
        <van-cell class="resoult_list" v-for="item in data.data">
            ...
            <div class="list_right" :style="{borderLeft:'2px solid '+eventType.data[item.eventType]}">
                <div class="list_right_context">{{item.eventUserName+item.fnaType+item.eventDescription}}</div>
                
                <div class="list_right_time">{{formater(item.eventStartTime)+'-'+formater(item.eventEndTime)}}</div>
            </div>
        </van-cell>
...
Toast('请稍后重试')
    ...
    console.log(data);
          ...
          return {
           data,getdata,fna,report,facilitate,eventType,formater
          }
